...
void Sort_countries(Country *c[], int*numberOfCountries);
...
int main(int argc, char*argv[])
{
	...
	Sort_countries(&pCountry, &numberOfCountries);
...
void Sort_countries(Country *c[], int *numberOfCountries)
{
    ...
    {
        for (int j = 0; j < *numberOfCountries - i - 1; ++j)
        {
            if ((*c)[j].population < (*c)[j + 1].population)
            {
                Country tempcountry = (*c)[j];
                (*c)[j] = (*c)[j + 1];
                (*c)[j + 1] = tempcountry;
            }
        }
